// Auto-generated from GarminExercisesCollector spreadsheet
// Source: https://github.com/maximecharriere/GarminExercisesCollector
// Total exercises: 2017

export const GARMIN_EXERCISES: Record<string, [string, string]> = {
  "banded ab twist": [
    "BANDED_EXERCISES",
    "AB_TWIST"
  ],
...
// Score how well two exercise names match based on word overlap
function scoreMatch(inputWords: Set<string>, candidateKey: string): number {
  ...
  for (const word of candidateWords) {
    if (inputWords.has(word)) {
      matchedWords++
      // Longer words are worth more (e.g., "deadlift" > "a")
      score += word.length
    }
  }

  // Bonus: if all input words matched, it's a strong match
  // Penalty: if candidate has many extra words, it's a weaker match
  const inputSize = inputWords.size
  const candidateSize = candidateWords.length

  // Require at least 1 word match
  if (matchedWords === 0) return 0

  // Calculate match ratio (how much of the input was matched)
  const matchRatio = matchedWords / Math.max(inputSize, candidateSize)

  // Final score combines word length score with match ratio
